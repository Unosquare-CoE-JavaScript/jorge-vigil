...
if (isMainThread) {
  ...
  const sharedMemory = new SharedArrayBuffer(
    syncOffset + THREADS * 4 // data + imageData
  );
  ...
  for (let x = 0; x < SIZE; x++) {
    for (let y = 0; y < SIZE; y++) {
      // 50% chance of cells being alive
  ...
  for (let i = 0; i < THREADS; i++) {
    const worker = new Worker('tread-gol.js', { name: `gol-worker-${i}` });
    worker.postMessage({
  ...
  self.addEventListener('message', initiListener);

  function initiListener(msg) {
    const opts = msg.data;
    sharedMemory = opts.sharedMemory;
    sync = new Int32Array(sharedMemory, syncOffset);
    self.removeEventListener('message', initiListener);
    if (opts.coord) {
